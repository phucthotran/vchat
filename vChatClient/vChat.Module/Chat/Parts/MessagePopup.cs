...
namespace vChat.Module.Chat.Parts
{
    public class MessagePopup
    {
        ...
        LinkedList<Popup> _popups = new LinkedList<Popup>();
        private double _latestHeight = 30;

        public static void Display(string content, Action click)
        {
            if (Instance._taskPopup == null || Instance._taskPopup.IsCompleted)
                Instance._taskPopup = Task.Factory.StartNew(() => { });
            Instance._taskPopup = Instance._taskPopup.ContinueWith(t => Instance.DisplayAsync(new object[] { content, click }), TaskScheduler.FromCurrentSynchronizationContext());
        }
